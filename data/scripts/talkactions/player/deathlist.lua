...
local deathlist = TalkAction("!deathlist")

function deathlist.onSay(player, words, param)
        ...
        if resultId ~= false then
            repeat
                ...
                str = str .. breakline .. " " .. date.day .. getMonthDayEnding(date.day) ..
                          ...
                          "   Died at Level " .. result.getNumber(resultId, "level") .. " by " ..
                          article .. killed_by .. "."
            until not result.next(resultId)
            result.free(resultId)
        end

        if str == "" then str = "No deaths." end
