...
function regenerateStamina.onLogin(player)
	if not configManager.getBoolean(configKeys.STAMINA_SYSTEM) then return true end
	...
	if offlineTime < 180 then return true end
	...
	if regainStaminaMinutes > maxNormalStaminaRegen then
		local happyHourStaminaRegen = (offlineTime - (maxNormalStaminaRegen * 180)) /
			                              600
		staminaMinutes = math.min(2520, math.max(2400, staminaMinutes) +
			                          happyHourStaminaRegen)
	else
