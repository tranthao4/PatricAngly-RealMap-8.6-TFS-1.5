local instruments = {
	...
	[2364] = {effect = CONST_ME_SOUND_GREEN}, -- post horn
	...
	[5786] = {
		effects = {failure = CONST_ME_SOUND_RED, success = CONST_ME_SOUND_YELLOW},
		monster = "war wolf",
		chance = 60,
		remove = true
	}, -- wooden whistle
...
function music.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local instrument, chance = instruments[item:getId()], nil
	...
	if not chance and instrument.remove then item:remove() end
