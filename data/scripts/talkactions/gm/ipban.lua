...
function ipban.onSay(player, words, param)
	local resultId = db.storeQuery(
		                 "SELECT `name`, `lastip` FROM `players` WHERE `name` = " ..
	...
	if targetIp == 0 then return false end
	...
	db.query(
		"INSERT INTO `ip_bans` (`ip`, `reason`, `banned_at`, `expires_at`, `banned_by`) VALUES (" ..
			targetIp .. ", '', " .. timeNow .. ", " .. timeNow + (ipBanDays * 86400) ..
			", " .. player:getGuid() .. ")")
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE,
...
ipban:separator(" ")
ipban:accountType(ACCOUNT_TYPE_GOD)
ipban:access(true)
ipban:register()
