...
-- StepIn event
local trapStepIn = MoveEvent()
trapStepIn:type("stepin")

function trapStepIn.onStepIn(creature, item, position, fromPosition)
...
trapStepIn:id(1510, 1513, 2579, 4208)
trapStepIn:register()

-- StepOut event
local trapStepOut = MoveEvent()
trapStepOut:type("stepout")

function trapStepOut.onStepOut(creature, item, position, fromPosition)
    if Tile(position):hasFlag(TILESTATE_PROTECTIONZONE) then
        return true
    end
...
trapStepOut:id(1511, 2578, 4209)
trapStepOut:register()

-- RemoveItem event
local trapRemove = MoveEvent()
trapRemove:type("removeitem")

function trapRemove.onRemoveItem(item, tile, position)
