...
function fireTiles.onStepIn(creature, item, position, fromPosition)
    ...
    if player:getVocation():getBase():getId() == fire.vocationId then
        doTargetCombatHealth(0, player, COMBAT_FIREDAMAGE, -300, -300, CONST_ME_FIREDAMAGE)
    else
        local combatType = COMBAT_FIREDAMAGE
        if fire.damage > 300 then
            combatType = COMBAT_PHYSICALDAMAGE
        end
        doTargetCombatHealth(0, player, combatType, -fire.damage, -fire.damage, CONST_ME_FIREDAMAGE)
    end
    return true
end

for aid, _ in pairs(fires) do
    fireTiles:aid(aid)
end
fireTiles:register()
