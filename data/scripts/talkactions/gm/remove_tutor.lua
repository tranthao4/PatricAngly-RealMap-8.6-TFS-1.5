...
function removeTutor.onSay(player, words, param)
    ...
    if target then
        ...
        db.query("UPDATE `accounts` SET `type` = " .. ACCOUNT_TYPE_NORMAL ..
            " WHERE `id` = " .. result.getNumber(resultId, "account_id"))
    end
...
removeTutor:accountType(ACCOUNT_TYPE_SENIORTUTOR)
removeTutor:access(true)
